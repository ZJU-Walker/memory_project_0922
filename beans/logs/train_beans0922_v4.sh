...
#   MODE=mem|smoke_mem [CFG=pi05_yam_beans0922_v4] [EXP=beans0922_v3] [JOB=<slurm job>] [GPUS=0,1] [GRES=<cards of the job>] [BATCH=8] [BATCH_FALLBACK="4"] bash beans/logs/train_beans0922_v3.sh
...
case $MODE in
  ...
  mem)        CFG=${CFG:-pi05_yam_beans0922_v4};  EXP=${EXP:-beans0922_v4};   BATCH=${BATCH:-8};  FALLBACK=${BATCH_FALLBACK:-4} ;;  # 2 x H200: 8 fits (peer ablation); H100 pair: 4
  smoke_mem)  CFG=${CFG:-pi05_yam_beans0922_v4}_smoke; EXP=${EXP:-smoke_mem_v4}; BATCH=${BATCH:-4}; FALLBACK=${BATCH_FALLBACK:-}; WANDB=0 ;;
...
# nvidia-smi ignores CUDA_VISIBLE_DEVICES: ask about OUR cards only (-i), or another job's process on the other cards of a shared
# 4-card allocation keeps us waiting forever (15:32 09-22: the user's Qwen job on GPUs 2,3 blocked the v3 launch)
gpu_busy() { local q=(nvidia-smi -i "$GPUS" --query-compute-apps=used_memory --format=csv,noheader,nounits)
  ...
  else "${q[@]}" 2>/dev/null | awk '$1+0>2000' | wc -l; fi; }
