...
log() { echo "[$(date +%m/%d\ %H:%M:%S)] switch5000v2: $*" | tee -a $ROOT/beans/logs/train_beans0922_status.log; }
...
# dataset via the sanctioned in-tree mirror dir local/ (a symlink to this node's /scr copy): the v3.5 authorization scan of stage 2
# refuses paths outside the tree, and local/ is accepted since 388b7b9
[ -d $ROOT/local/bean_scoop_0905_v5/meta ] || { log "local/bean_scoop_0905_v5 does not resolve on this node; not switching"; exit 1; }
cd $ROOT && export JOB=17489557 GPUS=0,1 WAIT_ROUNDS=2880 FREE_STREAK=2 OPENPI_BEANS_DATASET_ROOT=$ROOT/local/bean_scoop_0905_v5 && unset HF_DATASETS_CACHE && bash beans/logs/beans0922_ctl.sh start
