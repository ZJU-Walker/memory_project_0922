...
pids() { pgrep -u "$USER" -f "bash .*beans/ablations/(train_ablation[.]sh|run_${ROW:-[a-z0-9_]+}[.]sh)" | while read -r p; do
  [ -z "$ROW" ] || tr '\0' ' ' < /proc/$p/environ 2>/dev/null | grep -q "EXP=[a-z_]*${ROW}" || tr '\0' ' ' < /proc/$p/cmdline 2>/dev/null | grep -q "run_${ROW}[.]sh" || continue; echo "$p"; done; }
trains() { pgrep -u "$USER" -f "train[.]py pi05_yam_beans0922_ab_${ROW}"; }
case "${1:-status}" in
