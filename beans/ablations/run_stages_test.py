...
def test_mlp3_chain_owns_checkpoints_resumes_and_guards_batch(tmp_path, smoke, external_storage, explicit_run_name, row, a_steps, recipe):
    ...
    for stage in ("A", "B"):
        assert (scripts / f"logs/{run}_{stage}.recipe").read_text().startswith(
            f"{recipe} row={row} stage={stage} batch=12 accum=1"
        )
