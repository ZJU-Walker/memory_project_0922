...
while pgrep -u "$USER" -f "h100_onset_ab_input[s].sh" >/dev/null; do sleep 20; done
...
out="$R/beans/eval/flip_beans0922_v4c_2000_trainsplit"; rm -rf "$out"
srun --jobid=17489557 --overlap --nodes=1 --ntasks=1 --cpus-per-task=8 --gres=gpu:2 env CUDA_VISIBLE_DEVICES=0 \
  ...
  --params "$R/beans/checkpoints/pi05_yam_beans0922_v4c/beans0922_v4c/2000/params" --split train --batches 24 --batch-size 2 \
  --tokens sentence --output-dir "$out" > "$out.log" 2>&1
echo "battery TRAIN split v4c/2000 exit=$? $(date +%H:%M)" >> "$R/beans/eval/battery_in_job_status.log"
