#!/bin/bash
# onset A/B on H100 card 0 of job 17489557: python scripts/v5_onset_ab.py for one training episode of v4c/1750.
...
srun --jobid=17489557 --overlap --nodes=1 --ntasks=1 --cpus-per-task=8 --gres=gpu:2 env CUDA_VISIBLE_DEVICES=0 \
  ...
  --sidecar "$R/openpi/cluster_v5/beans/beans_v5_subtask_labels_0905_v7tgt.json" ${EXTRA:-} > "$R/beans/eval/onset_ab_ep$(printf %02d $EP)_$STEP.log" 2>&1
