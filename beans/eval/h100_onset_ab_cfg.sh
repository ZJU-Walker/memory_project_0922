#!/bin/bash
# onset A/B on H100 card 0 of job 17489557: python scripts/v5_onset_ab.py for one training episode of a chosen config/checkpoint (v4e loop).
...
srun --jobid=17489557 --overlap --nodes=1 --ntasks=1 --cpus-per-task=8 --gres=gpu:2 env CUDA_VISIBLE_DEVICES=0 \
  .venv/bin/python scripts/v5_onset_ab.py --config-name ${CFG:-pi05_yam_beans0922_v4e} \
  --params "$R/beans/checkpoints/${CFG:-pi05_yam_beans0922_v4e}/${EXPD:-beans0922_v4e}/$STEP/params" --episode-index "$EP" \
  --manifest "$R/openpi/cluster_v5/beans/beans_episode_manifest_0905_v1.json" \
  --sidecar "$R/openpi/cluster_v5/beans/beans_v5_subtask_labels_0905_v7tgt.json" ${EXTRA:-} > "$R/beans/eval/onset_ab_${CFGTAG:-v4e}_ep$(printf %02d $EP)_${STEP}${TAG:-}.log" 2>&1
