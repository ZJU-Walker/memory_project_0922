...
snap = the beans0922 **v3** structure since 2026-09-22 15:30 (user, relayed by the base session: "all ablation should adopt
same thing too"): v1 + beans0922_config.V3_QUERY_CONTEXT = change-only confident own writes (memory_v7_write_every_step False,
memory_v5_write_conf 0.9), "look before you ask" read questions (memory_v0920_query_context) and the error-driven sentence-token
weight 5 (memory_v7_hard_token_ce_weight). The sensory bank is unaffected by all three: it writes every valid tick (training
scan and serving transition gate it on tick validity, not on the sentence write decision), its fixed read queries are not
context-shifted, and the question context attends over the image + prompt tokens only (the sensory tokens are appended after
the sentence read). Rows started before this change (the 09-22 13:24 vis8 run, stopped at 420) used the v1 snap.
...
VIS_SLOTS = 8
SNAP_OVERRIDES = _b.V3_QUERY_CONTEXT  # the snap revision every row is built on (v3; see the module docstring)
