...
#   JOB=17329416 setsid nohup bash cluster_v6/gpu_sentinel_h200.sh > /dev/null 2>&1 < /dev/null &   (run ON iris-hgx-2)
...
echo "$(date '+%m/%d %H:%M') sentinel up for job $JOB on $(hostname) pid=$$ (tuned placeholder MEM_FRAC 0.5)" >> $LOG
while true; do
