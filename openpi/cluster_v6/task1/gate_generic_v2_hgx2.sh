#!/usr/bin/env bash
# Generic task1 gate v2 (sequential battery, waits for free H200 memory), run ON iris-hgx-2 (job 17329416, H200). Env: ACFG AEXP BCFG BEXP ENVVAR [ASTEPS] [MIN_RECALL=5].
...
battery() {  # modes run SEQUENTIALLY (one ~20 GB process at a time) unless PARALLEL=1
  for m in oracle oracle_evidence self; do
    ...
    if [ "${PARALLEL:-0}" = "1" ]; then MODES=$m JOB=17329416 GPU=0 bash $cv6/task1/run_task1_evals_hgx1.sh "$1" "$2" "$3" &
    else MODES=$m JOB=17329416 GPU=0 bash $cv6/task1/run_task1_evals_hgx1.sh "$1" "$2" "$3"; fi
  done
  wait
