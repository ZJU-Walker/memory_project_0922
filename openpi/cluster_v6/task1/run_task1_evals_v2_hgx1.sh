...
#   [JOB=17315830] [GPU=0] [MODES="self oracle"] [SIDECAR=<path>] [MANIFEST=<path>] cluster_v6/task1/run_task1_evals_v2_hgx1.sh <config-name> <exp-name> <step>
...
manifest="${MANIFEST:-$root/openpi/cluster_v6/task1/task1v6_episode_manifest_v1.json}"  # v2: env override (lead30 line)
sidecar="${SIDECAR:-$root/openpi/cluster_v6/task1/task1v6_v5_subtask_labels_v1.json}"
...
echo "battery start $(date +%m/%d\ %H:%M) host=$(hostname) ck=$ck modes=$modes sidecar=$(basename $sidecar)" >> "$out/status.log"
