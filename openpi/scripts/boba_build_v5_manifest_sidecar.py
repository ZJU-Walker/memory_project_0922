"""v5 generic episode manifest + sentence sidecar for the 0911 boba task (cluster_v7/README.md §3).

Same outputs and schemas as scripts/task1_build_v5_manifest_sidecar.py (openpi.v5.generic-manifest.v1 and
openpi.v5.subtask-labels.v1, self-hashed, pinned by SHA256 in the training config), built from
  ...
  * the converter manifest data/0911_boba_episode_manifest_v1.json (split, class, group per episode),
  * the per-episode label file subtask_labels.json of every raw demo (cluster_v6/boba/BOBA_LABELS.md v1).
The boba task has a single class (boba3_bean1) and no target/opened_bin: the sidecar episodes record
num_frames, class, group and the run-length segments only. The split is copied, never recomputed.
"""
...
def main() -> None:
    ...
    parser.add_argument("--episode-manifest", type=pathlib.Path, required=True,
                        help="converter manifest (data/0911_boba_episode_manifest_v1.json)")
    parser.add_argument("--out-dir", type=pathlib.Path, required=True)
    parser.add_argument("--dataset-name", default="0911_boba_v1")
    parser.add_argument("--manifest-name", default="boba_episode_manifest_v1.json")
    parser.add_argument("--sidecar-name", default="boba_v5_subtask_labels_v1.json")
    parser.add_argument("--split-seed", type=int, default=911,
                        help="integer seed the loader compares with memory_manifest_split_seed; the converter "
                             "split_rule must name it as sha256('0<seed>|...') or sha256('<seed>|...')")
    ...
    split_rule = str(conv.get("split_rule", ""))
    if f"sha256('{args.split_seed}|" not in split_rule and f"sha256('0{args.split_seed}|" not in split_rule:
        raise ValueError(f"--split-seed {args.split_seed} is not the seed named in the converter split_rule: {split_rule!r}")
    ...
    for index_str, source in sorted(sources.items(), key=lambda item: int(item[0])):
        ...
        label_file = raw_dir / str(source.get("label_file") or spec.get("label_file") or "subtask_labels.json")
        ...
        if int(source.get("num_frames", num_frames)) != num_frames:
            raise ValueError(f"{stable_id}: episode_sources has {source.get('num_frames')} frames, dataset {num_frames}")
    ...
    expected_vocab = conv.get("task_vocabulary")
    if expected_vocab is not None and sorted(expected_vocab) != sorted(vocabulary):
        raise ValueError(f"label vocabulary {sorted(vocabulary)} != converter manifest task_vocabulary {sorted(expected_vocab)}")
    ...
    manifest = {
        ...
        "split_rule": f"copied from {args.episode_manifest.name}: {split_rule}",
        "episodes": episodes,
    }
