...
# Needs a GPU with >= ~20 GB free: run it on the test job (default) once its placeholder is stopped.
set -u
# Default job: the user's designated test job 17405067 (1xH100 on iris-hgx-1, 2026-09-12 18:00); the 2xH100 17356154 is the
# user's own from 18:00. Its placeholder train.py must be stopped by explicit pid first (the 1 GB train_hs.py stays).
config="$1"; exp="$2"; step="$3"; JOB="${JOB:-17405067}"; GPU="${GPU:-0}"; modes="${MODES:-self oracle}"
